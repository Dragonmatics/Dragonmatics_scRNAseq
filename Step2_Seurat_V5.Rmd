...
# 📝 Metadata Integration and Sample Annotation
...
VlnPlot(seurat_obj, features = c("nCount_RNA", "nFeature_RNA", "percent.mt"),  split.by = "sampleID")
dev.off()
png(file.path(final_fig_dir, "violinplot.png"), width=15, height=10, units= "in", res = 300)
VlnPlot(seurat_obj, features = c("nCount_RNA", "nFeature_RNA", "percent.mt"),  split.by = "sampleID")
dev.off()
```
...
# ✂️ Subset the object based on QC metrics
...
# 🎶 Batch Effect Correction using Harmony Integration
...
# ⚙️ Data Integration using Reciprocal PCA (RPCA)
...
png(file.path(final_fig_dir, "elbowplot.png"), width=15, height=10, units = "in", res = 300)
ElbowPlot(seurat_obj, ndims = 50)
dev.off()
```

<div class="figure" style="text-align: center">
<img src="results/images/elbowplot.png" alt="Workflow of RNA-seq data analysis" width="90%" />
<p class="caption">**Figure 3:** Elbowplot</p>
</div>
...
png(file.path(final_fig_dir, "Rpca_Dimplot.png"), width=15, height=10, units = "in", res = 300)
DimPlot(rpca_seurat_obj, group.by = c("orig.ident", "rpca_clusters"), ncol = 2)
dev.off()
```
...
<p class="caption">**Figure 4:** RPCA integration results</p>
</div>




# 🏷️ Cell Population Identification and Annotation
...
png(file.path(final_fig_dir, paste0("annotated_res", i, "_Dimplot.png")), width=15, height=10, units = "in", res = 300)
DimPlot(seurat_obj, reduction = "umap_rpca", group.by = "rpca_clusters", label = TRUE)
dev.off()
```
...
<p class="caption">**Figure 5:** Clustering results at resolution 0.7</p>
</div>



# 🧑‍🏫 Expert-guided Cell Type Annotation
...
png(file.path(final_fig_dir, "annotated_Dimplot.png"), width=15, height=10)
DimPlot(seurat_obj, reduction = "umap_rpca", group.by = "annotated_celltype", label = TRUE)
dev.off()
```
...
<p class="caption">**Figure 6:** Annotated cell types on UMAP</p>
</div>
...
png(file.path(final_fig_dir, "GO_dotplot.png"), width=10, height=8, units = "in", res = 300)
dotplot(go_results, showCategory=20)
dev.off()
```
...
<p class="caption">**Figure 7:** GO enrichment analysis results</p>
</div>
