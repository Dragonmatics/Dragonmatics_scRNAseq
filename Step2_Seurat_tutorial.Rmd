...
# 🛠️ Setup the Seurat Object
This turorial is modified from [Seurat](https://satijalab.org/seurat/articles/pbmc3k_tutorial).
...
# ⚖️ Standard pre-processing workflow
...
# 🛳️ Perform linear dimensional reduction
...
```{r ridgeplot,  fig.height=6, fig.width=12}
...
```{r cellscatter, fig.height=8, fig.width=10}
...
```{r dotplot, fig.height=8, fig.width=10}
...
```{r save.img, include=TRUE,fig.height=5, fig.width=9 }
library(ggplot2)
